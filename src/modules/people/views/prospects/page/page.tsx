    ...
    <div
      ...
      <div className={"h-[60px] pt-5"}>
        {!open ? (
          <button
            className={
              "flex items-center justify-center rounded p-1 outline-none hover:bg-[#FFFFFF0F]"
            }
            onClick={() => {
              updateDrawerState(DrawerId.PRIMARY_SIDENAV, { open: true });
            }}>
            <CollapseRightIcon size={"16"} color={"#B3B3B3"} />
          </button>
        ) : null}
      </div>
      <div
        className={
          "mr-3 h-[calc(100%-72px)] rounded-md border border-solid border-[#292929] bg-bodyPrimary pl-2"
        }>
        People
      </div>
    </div>
